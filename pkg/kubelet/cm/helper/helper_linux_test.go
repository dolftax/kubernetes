...
func TestResourceConfigForPod(t *testing.T) {
	...
	testCases := map[string]struct {
		pod      *v1.Pod
		expected *cmtypes.ResourceConfig
	}{
		"besteffort": {
			pod: &v1.Pod{
				Spec: v1.PodSpec{
			...
			expected: &cmtypes.ResourceConfig{CpuShares: &minShares},
		},
		"burstable-no-limits": {
			pod: &v1.Pod{
				Spec: v1.PodSpec{
			...
			expected: &cmtypes.ResourceConfig{CpuShares: &burstableShares},
		},
		"burstable-with-limits": {
			...
			expected: &cmtypes.ResourceConfig{CpuShares: &burstableShares, CpuQuota: &burstableQuota, CpuPeriod: &burstablePeriod, Memory: &burstableMemory},
		},
		"burstable-partial-limits": {
			...
			expected: &cmtypes.ResourceConfig{CpuShares: &burstablePartialShares},
		},
		"guaranteed": {
			pod: &v1.Pod{
				Spec: v1.PodSpec{
			...
			expected: &cmtypes.ResourceConfig{CpuShares: &guaranteedShares, CpuQuota: &guaranteedQuota, CpuPeriod: &guaranteedPeriod, Memory: &guaranteedMemory},
		},
	}
